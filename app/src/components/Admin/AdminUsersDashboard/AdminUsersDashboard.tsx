...
export default function AdminUsersDashboard() {
  ...
  const [extraResourcesSaving, setExtraResourcesSaving] = useState<string | null>(null);
  ...
  async function handleRoleChange(userId: string, newRole: 'user' | 'admin' | 'superadmin') {
    ...
    try {
      ...
      notify(`Role updated to ${newRole.charAt(0).toUpperCase() + newRole.slice(1)}`, 'success');
    } catch {
  ...
  async function handlePlanAssign(userId: string, planId: string) {
    ...
    try {
      ...
      setUsers((prev) => prev.map((u) => (u._id === userId ? { ...u, plan: planId } : u)));
  ...
  function openExtraResourcesEdit(user: User) {
    setExtraResourcesEdit((prev) => ({
      ...prev,
      [user._id]: {
        requestsCpu: user.extraResources?.requests?.cpu?.toString() || '',
        requestsMemory: user.extraResources?.requests?.memory?.toString() || '',
        requestsStorage: user.extraResources?.requests?.storage?.toString() || '',
        limitsCpu: user.extraResources?.limits?.cpu?.toString() || '',
        limitsMemory: user.extraResources?.limits?.memory?.toString() || '',
        limitsStorage: user.extraResources?.limits?.storage?.toString() || '',
      },
    }));
  }
          ...
          allowedResources={users.reduce(
            (acc, user) => {
              ...
              if (planRes) {
                acc[user._id] = {
                  requests: {
                    cpu: sumNum(planRes.requests.cpu, extra.requests?.cpu),
                    memory: sumNum(planRes.requests.memory, extra.requests?.memory),
                    storage: sumNum(planRes.requests.storage, extra.requests?.storage),
                  },
                  limits: {
                    cpu: sumNum(planRes.limits.cpu, extra.limits?.cpu),
                    memory: sumNum(planRes.limits.memory, extra.limits?.memory),
                    storage: sumNum(planRes.limits.storage, extra.limits?.storage),
                  },
                };
              }
              return acc;
