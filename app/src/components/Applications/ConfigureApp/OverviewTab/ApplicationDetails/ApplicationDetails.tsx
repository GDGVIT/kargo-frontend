...
const ApplicationDetails: React.FC<{ form: Application | null }> = ({ form }) => {
  ...
  return (
    <>
      <h3 className="text-lg font-bold mb-2 text-gray-100">Application Details</h3>
      <Card className="bg-transparent">
        {/* Name and Image in one row */}
        <DetailItemRow>
          <DetailItem label="Name" value={form.name} />
          <DetailItem label="Image" value={`${form.imageUrl}:${form.imageTag}`} />
        </DetailItemRow>
        ...
        {(form.deploymentName || form.serviceName) && (
          <DetailItemRow>
            ...
            {form.serviceName && <DetailItem label="Service Name" value={form.serviceName} />}
          </DetailItemRow>
        )}
        ...
        {(form.createdAt || form.updatedAt) && (
          <DetailItemRow>
            {form.createdAt && (
              <DetailItem label="Created At" value={new Date(form.createdAt).toLocaleString()} />
            )}
            {form.updatedAt && (
              <DetailItem label="Updated At" value={new Date(form.updatedAt).toLocaleString()} />
            )}
          </DetailItemRow>
        )}
      </Card>
...
const DetailItemRow: React.FC<{ children: React.ReactNode }> = ({ children }) => (
...
const DetailItem: React.FC<{
  ...
  <div className={`detail-item flex flex-col items-start gap-1 flex-1 min-w-[120px] ${className}`}>
